...
class Player(pygame.sprite.Sprite):
    def __init__(self, pos, groups, collision_sprites, kill_sprites):
        super().__init__(groups)
        self.image = pygame.image.load(
            join("..", "graphics", "player", "walk", "1.png")
        )
        ...
        # timer
        self.timers = {"wall jump": Timer(100), "wall slide block": Timer(100)}
    ...
    def handle_kill_tiles(self):
        if pygame.sprite.spritecollideany(self, self.kill_sprites):
            self.death_count += 1  # Increment death counter
            print(f"Deaths: {self.death_count}")  # Log death count
            self.rect.topleft = self.starting_position  # Reset position
